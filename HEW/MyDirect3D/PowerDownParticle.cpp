...
#define POWERDOWN_PARTICLE_LIFE 50
#define ROTATION_VALUE		2 * D3DX_PI / 24.0f
#define HUE_VALUE			0.03f
#define HUE_STEP			0.1666667f

void PowerDownParticle::Initialize(LPDIRECT3DTEXTURE9 texture, D3DXVECTOR2 size) {
	m_pDevice = GetDevice();

	for (int nCntParticle = 0; nCntParticle < MAX_POWERDOWN_PARTICLE; nCntParticle++)
	{
	...
	m_enable = true;
	m_life = POWERDOWN_PARTICLE_LIFE;
	m_info.radius = 0;
	m_info.angle = 0;
...
void PowerDownParticle::Finalize() {
	SAFE_RELEASE(m_pD3DVtxBuffParticle);
	SAFE_RELEASE(m_pD3DTextureParticle);
}

HRESULT PowerDownParticle::MakeVertexParticle()
{
	// オブジェクトの頂点バッファを生成
	if (FAILED(m_pDevice->CreateVertexBuffer(sizeof(VERTEX_PARTICLE_3D) * NUM_VERTEX * MAX_POWERDOWN_PARTICLE,	// 頂点データ用に確保するバッファサイズ(バイト単位)
	...
	{//頂点バッファの中身を埋める
		...
		for (int nCntParticle = 0; nCntParticle < MAX_POWERDOWN_PARTICLE; nCntParticle++, pVtx += 4)
		{
			// 頂点座標の設定
...
// 頂点座標の設定
//=============================================================================
void PowerDownParticle::SetVertexParticle(int nIdxParticle, float fSizeX, float fSizeY)
{
	{//頂点バッファの中身を埋める
		VERTEX_PARTICLE_3D* pVtx;
...
// 頂点カラーの設定
//=============================================================================
void PowerDownParticle::SetColorParticle(int nIdxParticle, D3DXCOLOR col)
{
	{//頂点バッファの中身を埋める
		VERTEX_PARTICLE_3D* pVtx;
...
// 頂点情報の作成
//=============================================================================
int PowerDownParticle::SetParticle(D3DXVECTOR3 pos, D3DXVECTOR3 move, D3DXCOLOR col, int nLife)
{
	int nIdxParticle = -1;
...
void PowerDownParticle::SetPosition(D3DXVECTOR3 pos) {
	D3DXVECTOR3 newPos;
	newPos = pos;
	newPos.y += 2.0f;
	m_posBase = newPos;
...
int PowerDownParticle::GetLifeTime() {
	return m_life;
}

void PowerDownParticle::SetPlayerPosition(D3DXVECTOR3 pos) {
	m_playerPos = pos;
}

void PowerDownParticle::SetplayerAngle(float angle) {
	m_angle = angle;
}

void PowerDownParticle::UpdateLife() {
	m_life--;
	if (m_life <= 0)
	{
		m_enable = false;
...
void PowerDownParticle::UpdateParticle() {
	if (m_enable)
	{
		if (m_bPause == false)
		{
			for (int nCntParticle = 0; nCntParticle < MAX_POWERDOWN_PARTICLE; nCntParticle++)
			{
				if (particles[nCntParticle].bUse)
				{
					...
					particles[nCntParticle].col.r = 0.0f;
...
ParticleType PowerDownParticle::GetParticleType() {
