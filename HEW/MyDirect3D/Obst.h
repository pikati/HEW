#pragma once
#include "XManager.h"
#include "MyDirect3D.h"

enum OBST_TYPE{
	FIREWOOD,
	TREE,
	SANDSTORM,
	PITFALL,
	DRIFTWOOD,
	OBST_MAX
};

struct ObstacleInfo {
	D3DXVECTOR3 pos;
	D3DXVECTOR3 rot;
	D3DXVECTOR3 scale;
	OBST_TYPE	type;
	bool		enable;
	bool		hit;
	int			particleIndex;
	ObstacleInfo(D3DXVECTOR3 position, D3DXVECTOR3 rotation, D3DXVECTOR3 scl, OBST_TYPE newType, bool ena, bool hi, int particleIdx)
	{
		this->pos = position;
		this->rot = rotation;
		this->scale = scl;
		this->type = newType;
		this->enable = ena;
		this->hit = hi;
		this->particleIndex = particleIdx;
	}

	ObstacleInfo()
	{
		this->pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		this->rot = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		this->scale = D3DXVECTOR3(1.0f, 1.0f, 1.0f);
		this->type = FIREWOOD;
		this->enable = false;
		this->hit = false;
		this->particleIndex = -1;
	}
};

class Obst
{
protected:
	D3DXVECTOR3		m_pos;
	D3DXVECTOR3		m_rot;
	D3DXVECTOR3		m_scale;
	D3DXVECTOR3		m_playerPos;
	OBST_TYPE		m_elem;
	bool			m_bEnable;
	bool			m_bHit;

public:
	~Obst() {};
	virtual void	Initialize() = 0;
	virtual void	Update() = 0;
	virtual void	Hit() = 0;
	virtual bool	IsEnable() = 0;
	virtual void	CreateObstacle(D3DXVECTOR3 pos) = 0;
	virtual float*	CreateRondam(float* random) = 0;
	virtual void	SetPosition(D3DXVECTOR3 pos) = 0;
	virtual void	SetRotation(D3DXVECTOR3 rot) = 0;
	virtual void	SetScale(D3DXVECTOR3 scale) = 0;
	virtual D3DXVECTOR3	GetPosition() = 0;
	virtual D3DXVECTOR3	GetRotation() = 0;
	virtual D3DXVECTOR3	GetScale() = 0;
	virtual ObstacleInfo GetInfo() = 0;
	virtual void	Reset() = 0;
	virtual OBST_TYPE	GetElem() = 0;
	virtual void	SetPlayerPos(D3DXVECTOR3 pos) = 0;
};