...
#define MAX_NORMAL_PARTICLE 8
#define NOMALPARTICLE_LIFE 15

void NormalParticle::Initialize(LPDIRECT3DTEXTURE9 texture, D3DXVECTOR2 size) {
	m_pDevice = GetDevice();

	for (int nCntParticle = 0; nCntParticle < MAX_NORMAL_PARTICLE; nCntParticle++)
	{
...
void NormalParticle::Finalize() {
	SAFE_RELEASE(m_pD3DVtxBuffParticle);
	SAFE_RELEASE(m_pD3DTextureParticle);
}

HRESULT NormalParticle::MakeVertexParticle()
{
	// オブジェクトの頂点バッファを生成
	if (FAILED(m_pDevice->CreateVertexBuffer(sizeof(VERTEX_PARTICLE_3D) * NUM_VERTEX * MAX_NORMAL_PARTICLE,	// 頂点データ用に確保するバッファサイズ(バイト単位)
	...
	{//頂点バッファの中身を埋める
		...
		for (int nCntParticle = 0; nCntParticle < MAX_NORMAL_PARTICLE; nCntParticle++, pVtx += 4)
		{
			// 頂点座標の設定
...
// 頂点座標の設定
//=============================================================================
void NormalParticle::SetVertexParticle(int nIdxParticle, float fSizeX, float fSizeY)
{
	{//頂点バッファの中身を埋める
		VERTEX_PARTICLE_3D *pVtx;
...
// 頂点カラーの設定
//=============================================================================
void NormalParticle::SetColorParticle(int nIdxParticle, D3DXCOLOR col)
{
	{//頂点バッファの中身を埋める
		VERTEX_PARTICLE_3D *pVtx;
...
// 頂点情報の作成
//=============================================================================
int NormalParticle::SetParticle(D3DXVECTOR3 pos, D3DXVECTOR3 move, D3DXCOLOR col, int nLife)
{
	int nIdxParticle = -1;
...
void NormalParticle::SetPosition(D3DXVECTOR3 pos) {
	m_posBase = pos;
}

int NormalParticle::GetLifeTime() {
	return m_life;
}

void NormalParticle::SetPlayerPosition(D3DXVECTOR3 pos) {
	m_playerPos = pos;
}

void NormalParticle::SetplayerAngle(float angle) {
	m_angle = angle;
}

void NormalParticle::UpdateLife() {
	m_life--;
	if (m_life <= 0)
	{
		m_enable = false;
...
void NormalParticle::UpdateParticle() {
	if (m_enable)
	{
		if (m_bPause == false)
		{
			for (int nCntParticle = 0; nCntParticle < MAX_NORMAL_PARTICLE; nCntParticle++)
			{
				if (particles[nCntParticle].bUse)
				{
					...
					particles[nCntParticle].move.y -= 0.1f;
			...
			// パーティクル発生
			if (m_life == NOMALPARTICLE_LIFE - 1)
			{
				for (int i = 0; i < 10; i++)
				{
					D3DXVECTOR3 pos;
					...
					fLength = (rand() % (int)(m_fWidthBase * 200) / 100.0f - m_fWidthBase) * 0.1f;
...
ParticleType NormalParticle::GetParticleType() {
