...
void WaterParticle::Initialize(LPDIRECT3DTEXTURE9 texture, D3DXVECTOR2 size) {
	m_pDevice = GetDevice();
	...
	for (int nCntParticle = 0; nCntParticle < MAX_WATER_PARTICLE; nCntParticle++)
	{
...
void WaterParticle::Finalize() {
	SAFE_RELEASE(m_pD3DVtxBuffParticle);
	SAFE_RELEASE(m_pD3DTextureParticle);
}

HRESULT WaterParticle::MakeVertexParticle()
{
	// オブジェクトの頂点バッファを生成
	if (FAILED(m_pDevice->CreateVertexBuffer(sizeof(VERTEX_PARTICLE_3D) * NUM_VERTEX * MAX_WATER_PARTICLE,	// 頂点データ用に確保するバッファサイズ(バイト単位)
	...
	{//頂点バッファの中身を埋める
		...
		for (int nCntParticle = 0; nCntParticle < MAX_WATER_PARTICLE; nCntParticle++, pVtx += 4)
		{
			// 頂点座標の設定
...
// 頂点座標の設定
//=============================================================================
void WaterParticle::SetVertexParticle(int nIdxParticle, float fSizeX, float fSizeY)
{
	{//頂点バッファの中身を埋める
		VERTEX_PARTICLE_3D *pVtx;
...
// 頂点カラーの設定
//=============================================================================
void WaterParticle::SetColorParticle(int nIdxParticle, D3DXCOLOR col)
{
	{//頂点バッファの中身を埋める
		VERTEX_PARTICLE_3D *pVtx;
...
// 頂点情報の作成
//=============================================================================
int WaterParticle::SetParticle(D3DXVECTOR3 pos, D3DXVECTOR3 move, D3DXCOLOR col, int nLife)
{
	int nIdxParticle = -1;
...
void WaterParticle::SetPosition(D3DXVECTOR3 pos) {
	m_posBase = pos;
}

int WaterParticle::GetLifeTime() {
	return m_life;
}

void WaterParticle::SetPlayerPosition(D3DXVECTOR3 pos) {
	m_playerPos = pos;
}

void WaterParticle::SetplayerAngle(float angle) {
	m_angle = angle;
}

void WaterParticle::UpdateLife() {
	m_life--;
	if (m_life <= 0)
	{
		m_enable = false;
...
void WaterParticle::UpdateParticle() {
	if (m_enable)
	{
		m_life--;
		if (m_bPause == false)
...
ParticleType WaterParticle::GetParticleType() {
